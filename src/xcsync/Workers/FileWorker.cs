...
class FileWorker (ILogger Logger, IFileSystem fileSystem) : BaseWorker<FileMessage> {
	public override async Task ConsumeAsync (FileMessage message, CancellationToken cancellationToken = default)
	{
		try {
			...
			Logger.Information (Strings.Workers.WroteFile (data.Length, message.Path));

		} catch (Exception ex) {
			Logger.Fatal (ex, Strings.Workers.ProcessingException);
			throw;
		}
	}
	...
	{
		Log.Error (exception, Strings.Workers.ProcessingError);
...
class CopyFileWorker (ILogger Logger, IFileSystem fileSystem) : BaseWorker<CopyFileMessage> {
	public override Task ConsumeAsync (CopyFileMessage message, CancellationToken cancellationToken = default)
	{
		try {
			...
			Logger.Information (Strings.Workers.CopiedFile (message.sourcePath, message.destinationPath));
		} catch (Exception ex) {
			Logger.Fatal (ex, Strings.Workers.ProcessingException);
			throw;
		}
	...
	{
		Logger.Error (exception, Strings.Workers.ProcessingError);
		return Task.CompletedTask;
	}
}
