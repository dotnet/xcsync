...
class ContinuousSyncContext (IFileSystem fileSystem, ITypeService typeService, string projectPath, string targetDir, string framework, ILogger logger)
	...
	public async Task SyncAsync (CancellationToken token = default)
	{
		...
		await new SyncContext (FileSystem, TypeService, SyncDirection.ToXcode, ProjectPath, TargetDir, Framework.ToString (), Logger).SyncAsync (token);
	...
	protected async override Task ConfigureMarilleHub ()
	{
		...
		var worker = new ChangeWorker (FileSystem, TypeService, ProjectPath, TargetDir, Framework.ToString (), Logger, ClrProject, XcodeProject);
